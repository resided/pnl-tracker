...
export default function PNLTrackerApp() {
  ...
  // Determine if we should disable share (No data OR Gated)
  const isShareDisabled = !pnlData?.summary || isGated;
    ...
    <div style={{ minHeight: '100vh', background: colors.bg, fontFamily: 'system-ui, -apple-system, BlinkMacSystemFont, "SF Pro Text", sans-serif', color: colors.ink, position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ 
        ...
        <header style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '32px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            {/* UPDATED SHARE BUTTON: Disabled visually when gated */}
            <button 
              ...
              style={{ 
                ...
                alignItems: 'center', 
                gap: '6px', 
                cursor: isShareDisabled ? 'not-allowed' : 'pointer', // Cursor change
                opacity: isShareDisabled ? 0.5 : 1, // Visual greying out
                pointerEvents: isShareDisabled ? 'none' : 'auto'
              }}
            >
              <span>Share PnL</span>
            </button>

            {/* PNL PILL (Still visible to create "Teaser" effect, or we can grey it out too if you prefer) */}
            <div style={{ 
              padding: '4px 10px', 
              borderRadius: '999px', 
        ...
        {/* Tabs */}
        <div style={{ display: 'flex', gap: '8px', margin: '24px 0 16px', filter: isGated ? 'blur(5px)' : 'none' }}>
        ...
        {/* Lists */}
        <div style={{ filter: isGated ? 'blur(5px)' : 'none' }}>
